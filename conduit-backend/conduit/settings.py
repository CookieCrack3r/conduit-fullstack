...
# All settings below fall back to development-friendly defaults so that the
# project still runs locally without an .env file. Compose overrides them.
...
MIDDLEWARE = [
    ...
    # WhiteNoise serves the collected static files (including the admin CSS)
    # directly from gunicorn, so no separate web server is required for them.
    # It must sit directly below SecurityMiddleware.
...
# `collectstatic` gathers everything here at image build time; WhiteNoise then
# serves it. Kept out of git via .gitignore.
STATIC_ROOT = os.path.join(BASE_DIR, 'staticfiles')
STATICFILES_STORAGE = 'whitenoise.storage.CompressedManifestStaticFilesStorage'

# CORS origins are *browser* origins, i.e. wherever the frontend is served from.
# django-cors-middleware 1.3.1 expects bare "host:port" entries without a scheme.
